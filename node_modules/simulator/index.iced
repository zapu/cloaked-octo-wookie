...
class Project
	...
	find_work: (node) ->
		...
		for work in @workunits
			...
			work_score = work.get_best_total_score()# + work.get_assigned_score()
			opt_score = optimal.get_best_total_score()# + optimal.get_assigned_score()
		...
		if optimal
			node.assigned_work = optimal
			optimal.assign node, score
		return optimal
...
class Work
	...
	finish: (node, good = true) ->
		...
		if(!@done and total_score >= 1)
			@result_count = @results.length + @bad_count
...
nodes[0].historical_exp = 0.7 * Math.random()

nodes[1].accept_work = false
nodes[1].historical_exp = 0.7 * Math.random()

nodes[2].accept_work = false
...
makeResultsRow = () ->
	mapFn = (node) ->
		calc_corr = calculationCorrectnessScaled(node, proj)
		R_ex node.historical_exp, calc_corr
...
makeResultsRow = () ->
	results_count = 0
	results_sum = 0
	for workunit in proj.workunits
		if workunit.done 
			results_count += 1
			results_sum += workunit.result_count

	if results_count == 0
		return [0]
	else
		return [results_sum / results_count]


result_count_sum = 0

bad_result_ratio = 0.4
...
while work_done < proj.workunits.length
		...
		if node.busy > 0
			if(--node.busy <= 0)
				if node.assigned_work
					if node.assigned_work.finish(node, Math.random() >= bad_result_ratio)
						work_done++
	...
	resultRows.push makeResultsRow().join(',')
	tick++
	#console.log "#{tick}"

	if tick == 200
		nodes[2].accept_work = true
	else if tick == 400
		nodes[3].accept_work = false
	else if tick == 600
		nodes[1].accept_work = true
